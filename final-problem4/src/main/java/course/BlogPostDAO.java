...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
    	...
    	if (post != null) {
            ...
            if(comment.containsKey("num_likes")) 
            {
            	int numLike = comment.getInteger("num_likes");
            	comment.put("num_likes", ++numLike);
            }
            else
            {
    ...
    //	postsCollection.updateOne(eq("permalink",  permalink), new Document("$inc", new Document("comments.num_likes", 1)));
    }
}
